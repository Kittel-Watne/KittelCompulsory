...
EnemyCharacter::EnemyCharacter(const std::string& filename)
{
    ...
    //Position of enemy
    mMatrix.translate(0.5f, 0.2f, 2.0f);
    mMatrix.scale(0.5f);

}

bool EnemyCharacter::readObjFile(const std::string& filename)
{
...
void EnemyCharacter::walking() {
    move(mWalkSpeed);

    if(getPosition().x() < -1.5f)
        turnAround();
    if(getPosition().x() > 1.5f)
        turnAround();
}

void EnemyCharacter::turnAround() {
    mWalkSpeed = -mWalkSpeed;
}
